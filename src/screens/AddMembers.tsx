...
import { useAddMembersMutation, useGetUserContactsQuery } from "../api";

const AddMembers = ({ navigation, route }) => {
    const [searchTerm, setSearchTerm] = useState("");
    const [selectedContacts, setSelectedContacts] = useState({});
    const { chatId, userIds } = route?.params || {}
    ...
    const handleAddMembers = async () => {
        try {
            const body = { participants: Object.keys(selectedContacts)?.map((userId) => userId) }

            await addMembers({chatId, body}).unwrap();
    ...
    const renderItem = ({ item }) => {
        ...
        const alreadyInChat = !!userIds?.find((id) => id === item?.id);
        return (
            <TouchableOpacity style={styles.chatItem} disabled={alreadyInChat} onPress={() => toggleSelection(item)}>
                ...
                <View style={styles.chatDetails}>
                    <Text style={[styles.name, alreadyInChat && { color: COLORS.GRAY }]}>{item.name}</Text>
                    {alreadyInChat && <Text style={{
                        color: COLORS.GRAY,
                        fontSize: 14,
                        fontStyle: "italic"
                    }}>Already added to the group</Text>}
                </View>
            </TouchableOpacity>
        );
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            <SearchingHeader searchTerm={searchTerm} setSearchTerm={setSearchTerm} backHandler={() => navigation.goBack()} heading="Add Members" />
